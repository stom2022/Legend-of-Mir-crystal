        ...
        public void UseSpell(int key)
        {
            ...
            switch (magic.Spell)
            {
                ...
                case Spell.TwinDrakeBlade:
                    if (CMain.Time < ToggleTime) return;
                    actor.TwinDrakeBlade = !actor.TwinDrakeBlade;
                    // only show local chat for other actors; owner will receive server SpellToggle which updates and shows message
                    if (actor != User)
                        ChatDialog.ReceiveChat(
                            prefix + (actor.TwinDrakeBlade ? GameLanguage.ClientTextMap.GetLocalization(ClientTextKeys.UseTwinDrakeBlade) : GameLanguage.ClientTextMap.GetLocalization(ClientTextKeys.DoNotUseTwinDrakeBlade)),
                            ChatType.Hint);
                    ToggleTime = CMain.Time + 1000;
                    SendSpellToggle(actor, magic.Spell, actor.TwinDrakeBlade);
                    if (actor.TwinDrakeBlade)
                    {
                        if (actor == Hero)
